...
def load_data(data_path, data_index, relevant_markers, mode, skip_header=0):
    ...
    if mode == 'FCS':
        data_filename = data_path + '/sample' + str(data_index) + '.fcs'
...
def choose_reference_sample(data_path, data_index, relevant_markers, mode):
    samples = []
    for i in data_index:
        sample = load_data(data_path, i, relevant_markers, mode)
        sample = preprocess_samples(sample)
