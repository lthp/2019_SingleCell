...
def plot_progress(epoch, x1, x2, gx1, metrics, fname, dir_name='figures', tsne=True, umap=True):
    ...
    if epoch == 0:
        ...
        if umap:
            plot_umap(pd.concat([x1_plot, x2_plot]), save_as=os.path.join(fname, 'aegan_umap_x1-x2_epoch' + str(epoch)),
    ...
    if umap:
        plot_umap(pd.concat([x1_plot, gx1]), save_as=os.path.join(fname, 'aegan_umap_gx1-x1_epoch' + str(epoch)),
                  folder_name=dir_name)
        plot_umap(pd.concat([x2_plot, gx1]), save_as=os.path.join(fname, 'aegan_umap_gx1-x2_epoch' + str(epoch)),
                  folder_name=dir_name)
