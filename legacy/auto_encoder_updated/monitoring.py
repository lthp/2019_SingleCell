...
class monitorMMD(monitor):
    def __init__(self, inputData, inputLabel, MMDtarget, targetLabel, netMMDLayerPredict=None):
        self.MMDtarget = MMDtarget
        self.targetLabl = targetLabel
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        MMDtargetEmbeddingHandle = self.axFig3.scatter(self.MMDtargetEmbedding[:, 0], self.MMDtargetEmbedding[:, 1],
                                                       alpha=0.25, s=10, marker="o",
                                                       c=self.targetLabl, cmap='gist_rainbow',
        ...
        NetOuputHandle = self.axFig3.scatter(projection[:, 0], projection[:, 1],
                                             c=self.inputLabel, cmap='gist_rainbow',
                                             alpha=0.25, s=10, marker="v",
...
class monitorAnchor(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        targetEmbeddingHandle = self.axFig.scatter(self.targetEmbedding[:, 0], self.targetEmbedding[:, 1], marker="s",
        ...
        projection = np.dot(plotPredictions, self.pca.components_[[0, 1]].transpose())
        NetOuputHandle = self.axFig.scatter(projection[:, 0], projection[:, 1], marker=">",
